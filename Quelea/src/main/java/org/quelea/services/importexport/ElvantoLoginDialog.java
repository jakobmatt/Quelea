 ...
 */
public class ElvantoLoginDialog extends Stage {
    ...
    class ElvantoHttpHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange t) throws IOException {
            ...
            if (path.equals("/oauth")){
                String msg = LabelGrabber.INSTANCE.getLabel("elvanto.loginsuccess.message");
            ...
            if (params.containsKey("access_token")) {
                ...
                LOGGER.log(Level.WARNING, "Error logging into Elvanto", errorDesc);
                Dialog.showWarning(LabelGrabber.INSTANCE.getLabel("pco.loginerror.title"), LabelGrabber.INSTANCE.getLabel("elvanto.loginerror.warning") + ":" + errorDesc);
            }
    ...
    public ElvantoLoginDialog() {
        importDialog = null;
    }
    
    public ElvantoLoginDialog(ElvantoImportDialog importDlg) {
    ...
    public void start() {
        ...
        // host a local server to intercept the elvanto redirect
        // https://stackoverflow.com/questions/3732109/simple-http-server-in-java-using-only-java-se-api
        try {
            server = HttpServer.create(new InetSocketAddress(7070), 0);
            HttpContext context = server.createContext("/", new ElvantoHttpHandler());
        ...
        String url = "https://api.elvanto.com/oauth?type=user_agent&client_id=" + QueleaProperties.get().getElvantoClientId() + "&redirect_uri=http%3A%2F%2Flocalhost%3A7070%2Foauth&scope=ManageServices%2CManageSongs%2CManageCalendar";
